...
class NavigationUITests {
    ...
    @Test
    fun journalTest() {
        onView(allOf(isDescendantOfA(withId(R.id.nav_view)), withText(R.string.drawer_journal))).perform(click())
        onView(withId(R.id.text_journal)).check(matches(isDisplayed()))
    }

    @Test
    fun reportsTest() {
        onView(allOf(isDescendantOfA(withId(R.id.nav_view)), withText(R.string.drawer_reports))).perform(click())
        onView(withId(R.id.text_reports)).check(matches(isDisplayed()))
    }

    @Test
    fun calendarTest() {
        onView(allOf(isDescendantOfA(withId(R.id.nav_view)), withText(R.string.drawer_calendar))).perform(click())
        onView(withId(R.id.text_calendar)).check(matches(isDisplayed()))
    }

    @Test
    fun templatesTest() {
        onView(allOf(isDescendantOfA(withId(R.id.nav_view)), withText(R.string.drawer_templates))).perform(click())
